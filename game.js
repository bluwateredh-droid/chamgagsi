(() => {
  ...
  function drawHud(){
    const narrow=W<620,left=14+hudSafe.left,right=14+hudSafe.right,top=12+hudSafe.top,usableW=W-left-right,center=left+usableW/2,healthW=narrow?usableW:Math.min(300,usableW*.32),barX=left+44,barW=healthW-56;
    ...
    if(!narrow){ctx.fillStyle='#103d35e8';roundRect(center-78,top,156,40,12);ctx.font='900 21px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(formatTime(elapsed),center,top+27);ctx.fillStyle='#103d35e8';roundRect(W-right-176,top,176,40,12);ctx.font='bold 14px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(`Lv.${level}   🐛 ${kills}`,W-right-88,top+26);}else{ctx.font='900 17px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(formatTime(elapsed),center,top+62);ctx.font='bold 12px system-ui';ctx.fillText(`Lv.${level} · 🐛 ${kills}`,center,top+79);}
