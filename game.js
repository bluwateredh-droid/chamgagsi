(() => {
  ...
  const lotusColors=['#ffafd6','#fff0c7','#86d9f4','#c49cff','#ff8d72','#67dcb4','#f2cf55','#9276e8','#66788f','#ff496c'];
  const beeColors=['#ffd43f','#ffae3d','#f0e66b','#ef765f','#7ed9a0','#67bce8','#d38cf0','#9a7b55','#76808e','#ff4d4d'];
  const missions={
  ...
  function reset() {
    ...
    enterImmersive();document.body.classList.add('game-active');fitGameToViewport();initAudio(); startMusic(); requestAnimationFrame(loop);
  }
  function initAudio(){ if(!audioCtx) audioCtx=new (window.AudioContext||window.webkitAudioContext)(); if(audioCtx.state==='suspended') audioCtx.resume(); }
  function enterImmersive(){const el=document.documentElement;if(!document.fullscreenElement&&el.requestFullscreen)el.requestFullscreen({navigationUI:'hide'}).catch(()=>{});}
  ...
  function spawnEnemy(){
    ...
    const tough=Math.random()<Math.min(.38,elapsed/240), base=1+elapsed/240+Math.floor(elapsed/180)*.18,stage=Math.min(10,Math.floor(elapsed/180)+1),sizeScale=1+(stage-1)*.025;
    enemies.push({x,y,r:(tough?22:15)*sizeScale,hp:(tough?70:30)*base,maxHp:(tough?70:30)*base,speed:(tough?35:55)+Math.min(45,elapsed/8),type:tough?'beetle':'fly',stage,hit:0});
  }
  ...
  function defeatEnemyAt(index,e){
    kills++;const wasBoss=e.type==='boss';gems.push({x:e.x,y:e.y,r:wasBoss?10:6,value:wasBoss?15+e.bossStage*4:e.type==='beetle'?3:1,life:30});
    ...
    if((wasBoss&&Math.random()<.35)||(!wasBoss&&Math.random()<.004))items.push({x:ix,y:iy,type:'aqua',r:13,life:12});
    if(wasBoss||Math.random()<.012)items.push({x:ix-28,y:iy+18,type:'power',r:13,life:15});
  ...
  function update(dt){
    ...
    for(let i=gems.length-1;i>=0;i--){const g=gems[i];g.life-=dt;if(g.life<=0){gems.splice(i,1);continue;}const d=Math.hypot(g.x-player.x,g.y-player.y);if(d<player.magnet){const a=Math.atan2(player.y-g.y,player.x-g.x),sp=220+(player.magnet-d)*3;g.x+=Math.cos(a)*sp*dt;g.y+=Math.sin(a)*sp*dt;}if(d<player.r+8){xp+=g.value;gems.splice(i,1);beep(820,.04,'sine',.018);if(xp>=xpNeed){xp-=xpNeed;level++;xpNeed=Math.ceil(8+level*4.5+Math.pow(level,1.25));showLevelUp();break;}}}
  ...
  function drawEnemy(e){
    ctx.save();ctx.translate(screenX(e.x),screenY(e.y));const boss=e.type==='boss',tough=e.type==='beetle',visualStage=e.bossStage||e.stage||1,late=visualStage>=7;
    ...
    if(player.character==='frog'){
      const petals=boss?12:5+Math.floor((visualStage+1)/2)+(tough?2:0),petalColor=e.hit?'#fff':boss?`hsl(${(visualStage*34+300)%360} 85% 64%)`:lotusColors[visualStage-1];if(visualStage>=5&&!boss){ctx.globalAlpha=.2;ctx.fillStyle=petalColor;ctx.beginPath();ctx.arc(0,0,e.r+10+Math.sin(elapsed*5)*4,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}ctx.rotate(elapsed*(.25+visualStage*.025)*(boss?-1:1));for(let i=0;i<petals;i++){ctx.rotate(Math.PI*2/petals);ctx.fillStyle=petalColor;ctx.beginPath();ctx.ellipse(0,-e.r*(visualStage>=8?.68:.58),e.r*(visualStage>=6?.44:.36),e.r*(visualStage>=8?.82:.7),0,0,Math.PI*2);ctx.fill();if(visualStage>=4){ctx.strokeStyle=late?'#492342':'#87506f';ctx.lineWidth=2;ctx.stroke();}}ctx.fillStyle=visualStage>=9?'#331b35':visualStage>=6?'#ff8b3d':'#ffe77a';ctx.beginPath();ctx.arc(0,0,e.r*(tough?.48:.38),0,Math.PI*2);ctx.fill();if(visualStage>=7){ctx.fillStyle='#fff4a1';for(let i=0;i<4;i++){const a=i*Math.PI/2+elapsed;ctx.beginPath();ctx.arc(Math.cos(a)*e.r*.28,Math.sin(a)*e.r*.28,2.5,0,Math.PI*2);ctx.fill();}}ctx.rotate(-elapsed*(.25+visualStage*.025)*(boss?-1:1));
    }else{
      const bodyColor=e.hit?'#fff':boss?'#ff7b2f':beeColors[visualStage-1],wingPairs=visualStage>=7?2:1;if(visualStage>=5&&!boss){ctx.globalAlpha=.18;ctx.fillStyle=bodyColor;ctx.beginPath();ctx.arc(0,0,e.r+11+Math.sin(elapsed*7)*3,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}ctx.fillStyle=visualStage>=8?'#c7edffcc':'#dff7ffbb';for(let p=0;p<wingPairs;p++){const wy=-e.r*(.38-p*.48);ctx.beginPath();ctx.ellipse(-e.r*.58,wy,e.r*.58,e.r*.3,-.45,0,Math.PI*2);ctx.ellipse(e.r*.58,wy,e.r*.58,e.r*.3,.45,0,Math.PI*2);ctx.fill();}ctx.fillStyle=bodyColor;ctx.beginPath();ctx.ellipse(0,0,e.r,e.r*(visualStage>=6?.82:.72),0,0,Math.PI*2);ctx.fill();ctx.strokeStyle=late?'#17151b':'#322c21';ctx.lineWidth=boss?7:visualStage>=6?5:4;const stripes=visualStage>=5?4:3;for(let i=1;i<stripes;i++){const x=-e.r/2+i*e.r/stripes;ctx.beginPath();ctx.moveTo(x,-e.r*.68);ctx.lineTo(x,e.r*.68);ctx.stroke();}if(visualStage>=4){ctx.fillStyle=visualStage>=8?'#e53c3c':'#4b3425';ctx.beginPath();ctx.moveTo(e.r*.95,0);ctx.lineTo(e.r*1.48,-e.r*.18);ctx.lineTo(e.r*1.48,e.r*.18);ctx.closePath();ctx.fill();}ctx.fillStyle=visualStage>=9?'#ff3030':'#fff';ctx.beginPath();ctx.arc(-e.r*.35,-4,boss?7:4,0,Math.PI*2);ctx.arc(e.r*.35,-4,boss?7:4,0,Math.PI*2);ctx.fill();ctx.fillStyle='#28231d';ctx.beginPath();ctx.arc(-e.r*.32,-3,boss?3:2,0,Math.PI*2);ctx.arc(e.r*.32,-3,boss?3:2,0,Math.PI*2);ctx.fill();
    }
    if(!boss){ctx.textAlign='center';ctx.font='800 9px system-ui';ctx.fillStyle='#ffffffbb';ctx.fillText(`S${visualStage}`,0,e.r+13);}
  ...
  function drawHud(){
    const narrow=W<620,left=14+hudSafe.left,right=14+hudSafe.right,top=12+hudSafe.top,usableW=W-left-right,center=left+usableW/2,healthW=narrow?usableW:Math.min(300,usableW*.32),barX=left+44,barW=healthW-56,currentStage=Math.min(10,Math.floor(elapsed/180)+1);
    ctx.fillStyle='#103d35e8';roundRect(left,top,healthW,40,12);ctx.fillStyle='#582e38';roundRect(barX,top+12,barW,16,8);ctx.fillStyle='#ef6f6c';roundRect(barX,top+12,barW*Math.max(0,player.hp/player.maxHp),16,8);ctx.font='bold 13px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText('💚',left+13,top+27);ctx.textAlign='center';ctx.fillText(`${Math.ceil(player.hp)} / ${player.maxHp}`,barX+barW/2,top+25);
    if(!narrow){ctx.fillStyle='#103d35e8';roundRect(center-78,top,156,40,12);ctx.font='900 21px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(formatTime(elapsed),center,top+27);ctx.fillStyle='#103d35e8';roundRect(W-right-210,top,210,40,12);ctx.font='bold 13px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(`ST.${currentStage} · Lv.${level} · 🐛 ${kills}`,W-right-105,top+26);}else{ctx.font='900 17px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(formatTime(elapsed),center,top+62);ctx.font='bold 12px system-ui';ctx.fillText(`ST.${currentStage} · Lv.${level} · 🐛 ${kills}`,center,top+79);}
    const activeBoss=enemies.find(e=>e.type==='boss'),bossY=narrow?top+91:top+50,bossW=Math.min(usableW,480);if(activeBoss){ctx.fillStyle='#291b26e8';roundRect(center-bossW/2,bossY,bossW,35,10);ctx.fillStyle='#6b253e';roundRect(center-bossW/2+12,bossY+20,bossW-24,9,5);ctx.fillStyle='#ff626f';roundRect(center-bossW/2+12,bossY+20,(bossW-24)*Math.max(0,activeBoss.hp/activeBoss.maxHp),9,5);ctx.font='900 13px system-ui';ctx.fillStyle='#fff6d6';ctx.fillText(`${activeBoss.bossStage}차 보스 · ${activeBoss.name}`,center,bossY+15);}else if(sanctuary&&Math.hypot(player.x-sanctuary.x,player.y-sanctuary.y)<=sanctuary.r-player.r*.2){ctx.font='900 16px system-ui';ctx.fillStyle='#ffe45e';ctx.fillText('🛡️ 동부센트레빌 무적',center,bossY+18);}
    const xpX=left,xpY=H-24-hudSafe.bottom,xpW=usableW;ctx.fillStyle='#103d35e8';roundRect(xpX,xpY,xpW,14,7);ctx.fillStyle='#d3f269';roundRect(xpX,xpY,xpW*Math.min(1,xp/xpNeed),14,7);ctx.font='bold 10px system-ui';ctx.fillStyle='#fff';ctx.fillText(`${xp} / ${xpNeed} XP`,center,xpY+11);if(player.aquaInv>0){ctx.font='900 15px system-ui';ctx.fillStyle='#75eaff';ctx.fillText(`👟 무적 ${player.aquaInv.toFixed(1)}초`,center,xpY-10);}ctx.textAlign='left';
  }
  ...
  function fitGameToViewport(){
    ...
    let probe=document.querySelector('#safeAreaProbe');if(!probe){probe=document.createElement('div');probe.id='safeAreaProbe';probe.style.cssText='position:fixed;visibility:hidden;pointer-events:none;padding:env(safe-area-inset-top) env(safe-area-inset-right) env(safe-area-inset-bottom) env(safe-area-inset-left);';document.body.appendChild(probe);}const probeStyle=getComputedStyle(probe),safeLeft=parseFloat(probeStyle.paddingLeft)||0,safeRight=parseFloat(probeStyle.paddingRight)||0,safeTop=parseFloat(probeStyle.paddingTop)||0,safeBottom=parseFloat(probeStyle.paddingBottom)||0;
    ...
    const active=document.body.classList.contains('game-active'),headerH=header.getBoundingClientRect().height,footerH=coarse?0:footer.getBoundingClientRect().height,safeGap=active?0:coarse?6:18,maxW=Math.max(220,vw-(active?0:safeLeft+safeRight)-safeGap),maxH=Math.max(150,vh-(active?0:safeTop+safeBottom)-headerH-footerH-safeGap);
    ...
    const logicalH=600,logicalW=Math.max(240,Math.round(logicalH*gameW/gameH));if(canvas.width!==logicalW||canvas.height!==logicalH){canvas.width=logicalW;canvas.height=logicalH;W=logicalW;H=logicalH;}hudSafe={left:safeLeft*W/gameW,right:safeRight*W/gameW,top:safeTop*H/gameH,bottom:safeBottom*H/gameH};
    if(player)draw();
  }
  ...
  addEventListener('resize',fitGameToViewport);addEventListener('fullscreenchange',fitGameToViewport);addEventListener('orientationchange',()=>{fitGameToViewport();setTimeout(fitGameToViewport,250);setTimeout(fitGameToViewport,700);});if(window.visualViewport){window.visualViewport.addEventListener('resize',fitGameToViewport);window.visualViewport.addEventListener('scroll',fitGameToViewport);}
