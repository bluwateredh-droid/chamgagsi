(() => {
  ...
  let audioOn = true, audioCtx = null, bgmTimer = null, bgmStep = 0, touch = null, selectedCharacter = 'frog';
  ...
  function reset() {
    ...
    initAudio(); startMusic(); requestAnimationFrame(loop);
  }
  ...
  function update(dt){
    ...
    if(touch){dx=touch.x-W/2;dy=touch.y-H/2;const l=Math.hypot(dx,dy);if(l<12){dx=dy=0}else{dx/=l;dy/=l;}}
  ...
  function fitGameToViewport(){
    ...
    const headerH=header.getBoundingClientRect().height,footerH=coarse?0:footer.getBoundingClientRect().height,safeGap=coarse?6:18,maxW=Math.max(240,vw-safeGap),maxH=Math.max(150,vh-headerH-footerH-safeGap),ratio=W/H;
    let gameW=Math.min(maxW,1100),gameH=gameW/ratio;if(gameH>maxH){gameH=maxH;gameW=gameH*ratio;}
    gameW=Math.floor(gameW);gameH=Math.floor(gameH);shell.style.width=`${gameW+4}px`;wrap.style.width=`${gameW}px`;wrap.style.height=`${gameH}px`;
  }
